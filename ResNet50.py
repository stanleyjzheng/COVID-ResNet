...
def resNet(x):
    ...
    def resBlock(y, nb_channels_in, nb_channels_out, _strides=(1, 1), _project_shortcut=False):
        shortcut = y
        y = layers.Conv2D(nb_channels_in, kernel_size=(1, 1),
                          strides=(1, 1), padding='same')(y)
        y = commonLayers(y)
        y = groupedConvolution(y, nb_channels_in, _strides=_strides)
        y = commonLayers(y)
        y = layers.Conv2D(nb_channels_out, kernel_size=(
            1, 1), strides=(1, 1), padding='same')(y)
...
model.fit(images, labels, epochs=3, validation_data=(

...
def resNet(x):
    ...
    def resBlock(y, nb_channels_in, nb_channels_out, _strides=(1, 1), _project_shortcut=False):
        ...
        y = layers.Conv2D(nb_channels_in, kernel_size=(3, 3),
        ...
        if _project_shortcut or _strides != (1, 1):
            shortcut = layers.Conv2D(nb_channels_out, kernel_size=(
    ...
    for i in range(2):
        ...
        x = resBlock(x, 64, 64, _strides=strides,
...
opt = tf.keras.optimizers.SGD(
    learning_rate=1, nesterov=True, momentum=0.9, decay=0.01)
model.compile(
    optimizer=opt, loss=keras.losses.BinaryCrossentropy(), metrics=metrics)

model.fit(images, labels, epochs=35, validation_data=(
    verImg, verLabels), batch_size=32)

...
def makeDataset(workingDirectory=workingDirectory):
    ...
    for (index, row) in pd.read_csv(
        os.path.sep.join(
            [f"{workingDirectory}", "covid-chestxray-dataset", "metadata.csv"]
        )
    ).iterrows():
        if (
            row["finding"] != "COVID-19"
        ):  # Skip everything except if the case is COVID-19
            continue
    ...
    for (index, row) in pd.read_csv(
        os.path.sep.join(
            [f"{workingDirectory}", "Figure1-COVID-chestxray-dataset", "metadata.csv"]
        ),
        encoding="ISO-8859-1",
    ).iterrows():
        ...
        if os.path.exists(os.path.join(f"{fig1ImagePath}", row["patientid"] + ".jpg")):
            ...
            os.path.join(f"{fig1ImagePath}", row["patientid"] + ".png")
        ):
            singleImagePath = os.path.sep.join(
    ...
    for (index, row) in pd.read_csv(
        os.path.sep.join(
            [f"{workingDirectory}", "Actualmed-COVID-chestxray-dataset", "metadata.csv"]
        ),
    ...
    for (index, row) in pd.read_csv(
        os.path.sep.join([f"{workingDirectory}", "verification.csv"])
    ).iterrows():
        if row["finding"] != "COVID-19":
            continue
        ...
        if not os.path.exists(singleImagePath):
            print(
                "error making verification dataset. you may have a few missing verification images. They will be printed below."
            )
    ...
    print(
        "Number of verification images (should be 180)",
        str(len(list(paths.list_images(f"{verificationPath}")))),
    )
